...
/* Note: this plug-in is using TDF_VERBOSE to complete AST output */


int plugin_is_GPL_compatible;

static unsigned int walker_depth;	/* real walker depth */
static unsigned int walker_depth_print;	/* one used for pretty print */
...
static int ahg_ast_tiny_walker(tree , void *, walk_tree_nodes);
...
static int ahg_ast_tiny_walker(tree node, void *data,
				...
				void *data)
{
	...
	static int indirect_ref_parent = 0;
	...
	test_and_set_parent_cnt(&component_ref_parent);
	test_and_set_parent_cnt(&indirect_ref_parent);
	...
	*/
	...
	case LABEL_DECL:
		output_node_init(node, ast);
		output_label_declaration(buffer, node, ast->flags);
		output_char(buffer, ',');
		*walk_subtrees = 0;
		break;
	...
	case FIELD_DECL:
		...
		if (in_function_body &&
		    (code == FIELD_DECL ||
		     !component_ref_parent || !indirect_ref_parent))
			output_printf(buffer, "\",");
		*walk_subtrees = 0;
		break;
...
static struct plugin_data *ast_init(struct plugin_name_args *plugin_info)
{
	...
	l_ast->indent_level = &walker_depth_print;
